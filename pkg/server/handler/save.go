...
func SaveHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == "POST" {
		klog.Info("Handle post request of save xml")
		...
		if err != nil {
			klog.Error(err, "Fail to parse xml str")
			return
		}
		...
		minioClient, err := minio.New(
			...
			false)
		if err != nil {
			klog.Errorln(err, "Fail to connect minio")
			return
		}
		...
		if err != nil {
			klog.Errorln(err, "Fail to check bucket {}", bucketName)
			return
		}
		if !found {
			...
			if err != nil {
				klog.Errorln(err, "Fail to create bucket {}", bucketName)
				return
			}
			klog.Info("Successfully created {}.", bucketName)
		}
		...
		if err != nil {
			klog.Errorln(err, "Fail to put xml to minio {}/{}", bucketName, filename)
			return
		}
		klog.Infoln("Successfully uploaded bytes: {}", n)
		resp := SaveResponse{
			Code: 200,
			Msg:  "success",
		}
